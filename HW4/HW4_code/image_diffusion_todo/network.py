    ...
    def forward(self, x, timestep, class_label=None):
        ...
        if self.use_cfg and class_label is not None:
            if self.training:
                ...
                # Which samples to drop to null class
                drop_mask = torch.rand(class_label.shape, device=class_label.device) < self.cfg_dropout

                class_label = class_label.clone()  # Avoid modifying input tensor
                class_label[drop_mask] = 0
                #######################
